import { Badge } from '@/components/ui/badge';
import { ProductBadge } from '@/lib/types';

const badgeConfig: Record<ProductBadge, { label: string; className: string }> = {
  'best-seller': {
    label: 'Best Seller',
    className: 'bg-gold-500/15 text-gold-400 border-gold-500/30',
  },
  new: {
    label: 'New',
    className: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
  },
  popular: {
    label: 'Popular',
    className: 'bg-sky-500/15 text-sky-400 border-sky-500/30',
  },
  sale: {
    label: 'Sale',
    className: 'bg-rose-500/15 text-rose-400 border-rose-500/30',
  },
};

export function ProductBadgeDisplay({ badge }: { badge?: ProductBadge }) {
  if (!badge) return null;
  const config = badgeConfig[badge];
  return (
    <Badge
      variant="outline"
      className={`${config.className} backdrop-blur-md font-medium text-xs`}
    >
      {config.label}
    </Badge>
  );
}

export function RatingStars({ rating, size = 'sm' }: { rating: number; size?: 'sm' | 'md' }) {
  const starSize = size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4';
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg
          key={i}
          className={`${starSize} ${i <= Math.round(rating) ? 'text-gold-400' : 'text-white/15'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      ))}
    </div>
  );
}
